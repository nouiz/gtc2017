...
\begin{frame}{Some models build with Theano}
  ...
  %  # TODO updates!resnet, NiN, Unet, memory model
...
\begin{frame}{GPU: Theano flags}
  ...
  \item Instead of Theano flags, user can call ``theano.sandbox.cuda.cuda('cuda0')''
  \end{itemize}
\end{frame}
...
\begin{frame}{What is Lasagne}
  Lasagne is a thin framework on top of Theano.

  \begin{itemize}
  \item Do not hide Theano
  \item Easily build Theano graph by using layers
  \item Contain many preimplemented loss and optimizer
  \item Do not include a training loop
  \end{itemize}
\end{frame}
...
cat_cross_ent = lasagne.objectives.categorical_crossentropy
